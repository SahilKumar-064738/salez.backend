...
export class ContactsRepository {
   ...
   * Tenant-isolated. Soft-deleted contacts excluded.
   */
  async list(
    tenantId: number,
    ...
    // Resolve tag filter → contact IDs
    ...
    if (error) {
      logger.error({ error, tenantId }, 'contacts list failed');
      throw error;
    }
    ...
    const withTags: ContactWithTags[] = items.map((c) => ({
      ...c,
      tags: tagsMap[c.id] ?? [],
    }));
   ...
   * Returns all inserted rows with tags (empty arrays since new inserts have no tags).
   * Max 500 per batch — enforced upstream in the validation schema.
   */
  async bulkCreate(
    tenantId: number,
    contacts: Array<{
    ...
    return (data as Contact[]).map((c) => ({ ...c, tags: [] }));
  }
}
